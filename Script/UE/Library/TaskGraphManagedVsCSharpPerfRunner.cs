...
public static class TaskGraphManagedVsCSharpPerfRunner
{
        ...
        int rounds = 5)
    {
        ...
        for (var r = 0; r < rounds; r++)
        {
            ...
            if (orderIndex == 0)
            {
                order = "TG->PF->TR";
                ForceGc();
                tgMs = MeasureTaskGraphManaged(length, safeTaskCount, iterations, warmup, out sumTaskGraph);
                ForceGc();
            ...
            Console.WriteLine(
                $"[ManagedPerfRound] round={r + 1}/{rounds} order={order} " +
        ...
        Console.WriteLine(
            $"[ManagedPerfSummary] length={length} taskCount={safeTaskCount} iterations={iterations} warmup={warmup} rounds={rounds} " +
    ...
    private static double MeasureTaskGraphManaged(
        int length,
        int taskCount,
        int iterations,
        int warmup,
        out long sum)
    {
        ...
        {
            sum = RunTaskGraphOnce(data, taskCount);
        }
    ...
    private static long RunTaskGraphOnce(int[] data, int taskCount)
    {
        var len = data.Length;
        var chunkSize = (len + taskCount - 1) / taskCount;
        long sum = 0;

        TaskGraphBatch.ExecuteBatch(index =>
        {
